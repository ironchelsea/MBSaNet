...
def res_MBConv(inputs, output_channel, conv_short_cut=True, strides=1, expansion=4, se_ratio=0, drop_rate=0, activation="gelu", name=""):
    ...
    if conv_short_cut:
        # Avg or Max pool
        shortcut = conv2d_no_bias(inputs, output_channel, 2, strides=2, padding='valid', name=name + "shortcut_conv_pool") if strides > 1 else inputs
...
def res_ffn(inputs, expansion=4, kernel_size=1, drop_rate=0, activation="relu", name=""):
    """ x ← x + Module (Norm(x)) """
    # preact
    # preact = batchnorm_with_activation(inputs, activation=None, zero_gamma=False, name=name + "preact_")
...
def Model(
    ...
    branch1 = conv2d_no_bias(inputs, stem_width // 4, 3, strides=2, padding='same', name="stem_1_branch_1_")
    branch1 = batchnorm_with_activation(branch1, activation=activation, name="stem_1_branch_1_")
    branch2 = conv2d_no_bias(inputs, stem_width // 4, 5, strides=2, padding='same', name="stem_1_branch_2_")
    branch2 = batchnorm_with_activation(branch2, activation=activation, name="stem_1_branch_2_")
    branch3 = conv2d_no_bias(inputs, stem_width // 4, 7, strides=2, padding='same', name="stem_1_branch_3_")
    branch3 = batchnorm_with_activation(branch3, activation=activation, name="stem_1_branch_3_")
    branch4 = conv2d_no_bias(inputs, stem_width // 4, 9, strides=2, padding='same', name="stem_1_branch_4_")
    branch4 = batchnorm_with_activation(branch4, activation=activation, name="stem_1_branch_4_")
    nn = tf.keras.layers.concatenate([branch1, branch2, branch3, branch4])
...
def MBSaNet(input_shape=(224, 224, 3), num_classes=1000, activation="relu", classifier_activation="sigmoid", **kwargs):
    num_blocks = [2, 3, 5, 2]
    out_channels = [64, 128, 256, 512]
    stem_width = 64
    return Model(**locals(), model_name="MBSaNet", **kwargs)